...
class Sprite {
    ...
    draw(ctx: CanvasRenderingContext2D, cameraPerson: GameObject) {
        ...
        const x = this.gameObject.x - UTILS.nudgeSprite.person.x + UTILS.withGrid(UTILS.cameraPersonOffset.x) - cameraPerson.x;
        /** y position to draw this sprite accounting for camera position and nudging to center of tile */
        const y = this.gameObject.y - UTILS.nudgeSprite.person.y + UTILS.withGrid(UTILS.cameraPersonOffset.y) - cameraPerson.y;
